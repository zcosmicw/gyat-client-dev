...
public class GuiGraphics {
    ...
    public void fillRounded(int x, int y, int width, int height, int cornerRadius, int color) {
        fill(x + cornerRadius, y, x + width - cornerRadius, y + cornerRadius, color);
        fill(x + cornerRadius, y + height - cornerRadius, x + width - cornerRadius, y + height, color);
        fill(x, y + cornerRadius, x + cornerRadius, y + height - cornerRadius, color);
        fill(x + width - cornerRadius, y + cornerRadius, x + width, y + height - cornerRadius, color);
        fill(x + cornerRadius, y + cornerRadius, x + width - cornerRadius, y + height - cornerRadius, color);
        drawQuarterCircle(x + cornerRadius, y + cornerRadius, cornerRadius, 0, color);
        drawQuarterCircle(x + width - cornerRadius, y + cornerRadius, cornerRadius, 1, color);
        drawQuarterCircle(x + cornerRadius, y + height - cornerRadius, cornerRadius, 2, color);
        drawQuarterCircle(x + width - cornerRadius, y + height - cornerRadius, cornerRadius, 3, color);
    }

    protected void drawQuarterCircle(int centerX, int centerY, int radius, int quadrant, int color) {
        for (int i = 0; i < radius; i++) {
            for (int j = 0; j < radius; j++) {
                if (i * i + j * j <= radius * radius) {
                    int x = centerX + (quadrant == 1 || quadrant == 3 ? i : -i - 1);
                    int y = centerY + (quadrant == 2 || quadrant == 3 ? j : -j - 1);
